...
_init_state('transitions_list', [{"source":"Healthy","target":"Dead","probability":1.0}])
...
# --- Helpers for file-based configs ---
...
# --- Helpers for file-based results ---
...
def apply_uploaded_config():
    ...
    if not isinstance(cfg, dict):
        st.error("Uploaded config invalid.")
        return
...
if page == "Draw":
    # Prepare Mermaid
    mer_lines = ["graph TD", f"    Start -- [{st.session_state.draw_num_patients}] --> Healthy;"]
    probs = {}
    ...
    if st.session_state.show_left:
        with colL:
            ...
            with setup_tab:
                ...
                st.file_uploader("Choose YAML file", type=["yaml","yml"], key="uploaded_config_file", on_change=handle_uploaded_config)
                ...
                with cb:
                    ...
                    if st.button("Save Config") and save_name:
                        cfg = {"model_name":st.session_state.draw_model_name,"initial_patients":st.session_state.draw_num_patients,"transitions":st.session_state.transitions_list,"timestep_unit":st.session_state.timestep_unit}
                ...
                st.text_input("Model Name", value=st.session_state.draw_model_name, key="draw_model_name")
                st.number_input("Initial Number of People", value=st.session_state.draw_num_patients, min_value=1, key="draw_num_patients")
                st.selectbox("Timestep Unit", ["Year","Month","Week","Day"], index=["Year","Month","Week","Day"].index(st.session_state.timestep_unit), key="timestep_unit")
                st.markdown("---")
                # Download current config
                cur = {"model_name":st.session_state.draw_model_name,"initial_patients":st.session_state.draw_num_patients,"transitions":st.session_state.transitions_list,"timestep_unit":st.session_state.timestep_unit}
                st.download_button("Download Current Configuration (YAML)", data=yaml.dump(cur), file_name="config.yaml", mime="text/yaml")
            ...
            with edit_tab:
                ...
                for i,t in enumerate(st.session_state.transitions_list):
                    ...
                    with c3: t['probability']=st.number_input(
                        "Probability",
                        value=t['probability'],
                        min_value=0.0,
                        max_value=1.0,
    ...
    if st.session_state.show_right:
        with colR:
            ...
            import m from 'https://unpkg.com/mermaid@10/dist/mermaid.esm.min.mjs';m.initialize({{startOnLoad:true}});
            </script></head><body>
    ...
    with tab2:
        if st.session_state.loaded_config_name:
            ...
            sim_initial=st.number_input("Initial Number of People",value=st.session_state.loaded_config_data.get('initial_patients',1000),min_value=1,key="sim_initial")
            ...
            sim_unit=st.selectbox("Timestep Unit",["Year","Month","Week","Day"],index=["Year","Month","Week","Day"].index(st.session_state.loaded_config_data.get('timestep_unit','Week')),key="sim_unit")
            if st.button("Start Simulation"):
                trans=st.session_state.loaded_config_data.get('transitions',[])
                nodes={t['source'] for t in trans}|{t['target'] for t in trans}
                current={n:0 for n in nodes};current['Healthy']=sim_initial;current.pop('Start',None)
                cols=sorted(current.keys())
                history=pd.DataFrame(columns=['Step']+cols);history.loc[0]=[f"{sim_unit} 0"]+[current[c] for c in cols]
                prog=st.progress(0);stat=st.empty();g=st.empty()
                for i in range(1,sim_steps+1):
                    infl={c:0 for c in cols};out={c:0 for c in cols}
                    for tr in trans:
                        m=round(current.get(tr['source'],0)*tr['probability']);out[tr['source']]+=m;infl[tr['target']]+=m
                    for c in cols:current[c]=max(0,current[c]+infl[c]-out[c])
                    history.loc[i]=[f"{sim_unit} {i}"]+[current[c] for c in cols]
                    prog.progress(i/sim_steps);stat.text(f"Step {i}/{sim_steps}")
                    dot=graphviz.Digraph(graph_attr={'rankdir':'LR'},node_attr={'shape':'ellipse'})
                    dot.node('Start','Start')
                    for c in cols:dot.node(c,f"{c}\n({current[c]} patients)")
                    dot.edge('Start','Healthy',label=str(sim_initial))
                    for tr in trans:dot.edge(tr['source'],tr['target'],label=str(tr['probability']))
                    g.graphviz_chart(dot,use_container_width=True)
                    time.sleep(sim_speed)
                st.session_state.sim_results_df = history
                # Use the custom name if provided, otherwise default prefix
    ...
    with tab3:
        ...
        if st.session_state.sim_results_df is not None:
            ...
            st.download_button("Download Current Results",data=csv,file_name=f"{st.session_state.loaded_config_name}_results.csv",mime="text/csv")
        past=list_results()
        if past:
            sel2=st.selectbox("Load Past Results",options=past,key="res_sel")
            if sel2:
                df=load_results_file(sel2);st.dataframe(df,use_container_width=True)
                st.download_button("Download CSV",data=df.to_csv(index=False).encode(),file_name=sel2,mime="text/csv")
        else:
